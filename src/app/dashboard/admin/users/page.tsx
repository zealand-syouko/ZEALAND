    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-white shadow overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>{users.map((u) => (<tr key={u.id} className="border-b"><td className="px-4 py-3">{u.email}</td><td className="px-4 py-3 font-bold">{u.balance.toLocaleString()}</td><td className="px-4 py-3">{u.verified ? 'Yes' : 'No'}</td><td className="px-4 py-3 text-gray-500">{new Date(u.createdAt).toLocaleDateString()}</td><td className="px-4 py-3">{u.email !== "admin@tokenrelay.local" && <button onClick={async () => { if (confirm('Delete user ' + u.email + '?')) { await fetch('/api/admin/users', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: u.id }) }); fetchUsers(); } }} className="text-red-500 hover:underline text-xs">Delete</button>}</td></tr>))}</tbody>
        </table>
      </div>
    </div>
    </AdminGuard>
