...
export default function AdminPricingPage() {
  ...
  async function handleSave() { if (!model || !inputPrice || !outputPrice) return; setSaving(true); await fetch("/api/admin/pricing", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model, inputPrice: Math.round(Number(inputPrice) * 100), outputPrice: Math.round(Number(outputPrice) * 100) }) }); setModel(""); setInputPrice(""); setOutputPrice(""); setSaving(false); fetchPricing(); }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500">Prices are in USD per 1,000,000 tokens. Model matches by prefix (e.g., "deepseek" matches deepseek-chat).</p>
      <div className="flex gap-3 items-end">
        ...
        <div><label className="block text-sm text-gray-500 mb-1">{t("inputPrice")} ($/1M)</label><input value={inputPrice} onChange={(e) => setInputPrice(e.target.value)} placeholder="0.50" type="number" step="0.01" className="rounded border px-3 py-2 text-sm w-24" /></div>
        <div><label className="block text-sm text-gray-500 mb-1">{t("outputPrice")} ($/1M)</label><input value={outputPrice} onChange={(e) => setOutputPrice(e.target.value)} placeholder="1.40" type="number" step="0.01" className="rounded border px-3 py-2 text-sm w-24" /></div>
      ...
      <div className="rounded-xl bg-white shadow overflow-hidden">
        <table className="w-full text-sm" suppressHydrationWarning>
          <thead><tr className="text-left text-gray-500 border-b"><th className="px-4 py-3">{t("model")}</th><th className="px-4 py-3">Input $/1M</th><th className="px-4 py-3">Output $/1M</th><th className="px-4 py-3">{t("actions")}</th></tr></thead>
          <tbody>
            {pricing.map((p) => (
              <tr key={p.id} className="border-b">
                <td className="px-4 py-3 font-mono">{p.model}</td>
                <td className="px-4 py-3">${(p.inputPrice / 100).toFixed(2)}</td>
                <td className="px-4 py-3">${(p.outputPrice / 100).toFixed(2)}</td>
