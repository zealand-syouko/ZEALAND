...
export async function POST(req: NextRequest) {
  ...
  const config = { apiKey, baseUrl: providerRecord.baseUrl || "" };
  ...
  try {
    if (body.stream) {
      ...
      createCallLog({
        apiKeyId: auth.apiKey.id, provider: providerName, model: body.model,
        endpoint: "/v1/chat/completions", requestTokens: 0, responseTokens: 0, totalTokens: 0,
        latencyMs: Date.now() - start, status: 200,
      }).catch(() => {});
      ...
      createCallLog({
        apiKeyId: auth.apiKey.id, provider: providerName, model: body.model,
        endpoint: "/v1/chat/completions", requestTokens: inputTokens, responseTokens: outputTokens,
        totalTokens, latencyMs: latency, status: 200,
      }).catch(() => {});
      incrementApiKeyQuota(auth.apiKey.id, totalTokens).catch(() => {});
      chargeUser(auth.apiKey.userId, body.model, inputTokens, outputTokens, auth.apiKey.id).catch(() => {});
    ...
    createCallLog({
      apiKeyId: auth.apiKey.id, provider: providerName, model: body.model,
      endpoint: "/v1/chat/completions", requestTokens: 0, responseTokens: 0, totalTokens: 0,
      latencyMs: latency, status: 502, errorMessage: message,
    }).catch(() => {});

    return NextResponse.json({ error: { message, type: "upstream_error" } }, { status: 502 });
  }
}
